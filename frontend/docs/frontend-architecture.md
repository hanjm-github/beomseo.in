...
## 6.3 정적 평가계획서 다운로드 흐름

평가계획서 다운로드 화면은 백엔드 API 없이 Vite public 디렉터리의 원본 HWP 파일을 직접 링크합니다.

```mermaid
flowchart LR
    A["public/evaluation-plans/*.hwp"] --> B["EvaluationPlansPage"]
    C["public/kogl/img_opentype03.jpg"] --> B
    B --> D["브라우저 직접 다운로드"]
```
