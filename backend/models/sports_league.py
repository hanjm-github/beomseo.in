...
class SportsLeagueCategory(db.Model):
    ...
    # Player rosters are stored outside the event snapshot so lineup/ranking updates can use focused queries.
...
class SportsLeagueTeam(db.Model):
    ...
    # Team-level access is used by the lineup UI and by category-scoped player sorting.
    players = db.relationship(
        'SportsLeaguePlayer',
        backref='team',
        cascade='all, delete-orphan',
...
class SportsLeaguePlayer(db.Model):
    # This table is intentionally small and category-scoped because the UI always reads one league at a time.
    ...
    team_id = db.Column(
        db.String(120),
        db.ForeignKey('sports_league_teams.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    name = db.Column(db.String(20), nullable=False)
    goals = db.Column(db.Integer, nullable=False, default=0, index=True)
    assists = db.Column(db.Integer, nullable=False, default=0, index=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(
        db.DateTime,
        default=datetime.utcnow,
    ...
    __table_args__ = (
        # Category-first indexes keep lineup and ranking queries cheap without coupling them to match events.
        db.Index('ix_sports_league_players_category_team', 'category_id', 'team_id'),
        db.Index('ix_sports_league_players_category_goals', 'category_id', 'goals'),
        db.Index('ix_sports_league_players_category_assists', 'category_id', 'assists'),
    )
