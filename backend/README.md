...
flowchart LR
    ...
    FastAPI["FastAPI Server<br/>:8000 (스포츠리그 + 수학여행 + 급식)"]
    MariaDB[("MariaDB")]
    Redis[("Redis")]
...
|---|---|
| Language | Python |
| Web Framework | Flask 3.1 (메인 API), **FastAPI** (스포츠리그 + 수학여행 + 급식 전용) |
...
- 제공 범위: `/api/sports-league/*`, `/api/community/field-trip/*`, `/api/school-info/meals*`
...
- 급식 읽기/평점 엔드포인트는 `MEAL_RATING_COOKIE_NAME` 쿠키를 공유해 비로그인 브라우저도 날짜/카테고리별 1개의 평점을 유지합니다.
- 급식 알림 구독은 계정이 아니라 `installationId` 기준의 설치 기기 단위 레코드입니다.
...
- 급식 읽기와 평점 쓰기는 모두 브라우저별 익명 식별 쿠키(`MEAL_RATING_COOKIE_NAME`)를 공유하므로, 비로그인 사용자도 날짜/카테고리별로 1개의 평점을 유지합니다.
- 알림 구독은 사용자 계정이 아니라 `installationId` 기준의 기기 단위 레코드로 저장됩니다.
- 요청 경로는 MySQL에 저장된 급식 데이터만 읽고, 실제 NEIS 호출은 `scripts/sync_school_meals.py`에서만 수행합니다.
- 동기화 진입점: `python scripts/sync_school_meals.py --year current`
- 알림 발송 진입점: `python scripts/send_school_meal_notifications.py`
- 상세 문서: [docs/school_meals.md](./docs/school_meals.md)
- 데이터 소스: 공식 NEIS `mealServiceDietInfo`
- Cron 예시:
