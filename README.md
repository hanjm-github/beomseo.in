...
학생들이 학교 생활에서 필요한 정보를 한곳에서 쉽게 찾고, 서로 소통할 수 있는 공간을 만들기 위해 시작되었습니다. 학교/학생회 공지, 범서고·천상고 학교 소개, 예산 공개 게시판 확인부터 자유 게시판, 동아리 모집, 학생 청원, 설문조사, 실시간 투표, 분실물 게시판, 교내 중고거래, 수학여행 반별 미션 게시판, 그리고 학교 생활 정보 허브 안의 QR 코드 생성기·샤니마스 카드 생성기·평가계획서 다운로드·스포츠리그 문자중계·팀별 라인업·개인별 순위까지 — 범서고 학생이라면 누구나 참여할 수 있습니다.
...
- **학교 소개** — 범서고·천상고 소개 탭과 범서고 공식 자료 출처 링크
...
- **학교 생활 정보 허브** — QR 코드 생성기, 샤니마스 카드 생성기, 시간표 다운로드, 평가계획서 다운로드, 학사 캘린더, 급식 조회/평점/PWA 알림, 스포츠리그 문자중계/라인업/개인 순위
...
| 아이콘 | **Lucide React** | |
| QR 생성 | **react-qrcode-logo** | 브라우저 캔버스 기반 QR 생성/다운로드 |
| 카드 렌더링 | **react-shany-card-generator** | 샤니마스 스타일 이름 레이어·카드 합성 이미지 생성/다운로드 |
| XSS 방어 | **DOMPurify** | |
...
| [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) | 공공누리 제3유형 등 별도 라이선스 자료 고지 |

## 📜 라이선스

이 프로젝트는 [**GNU General Public License v3.0**](LICENSE) 하에 배포됩니다.
단, `frontend/public/evaluation-plans/`의 평가계획서 HWP 파일과 `frontend/public/kogl/img_opentype03.jpg`는 공공누리 제3유형 조건으로 제공되는 별도 자료이며 GPL-3.0으로 재라이선스되지 않습니다. 자세한 출처와 조건은 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)를 확인하세요.
